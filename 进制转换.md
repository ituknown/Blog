...
不同于二进制，对于八进制和十六进制在各种编程语言中是可以直接书写的。八进制的数字范围是 0-7，满 8 进 1。在各大编程语言中尝使用 0 做前缀表示八进制，比如 010 就是八进制表示法，它对应的阿拉伯数字是 8，011 自然就是 9 了。

十六进制使用 0-9 及 A-F 表示。A 表示的就是十进制的 10，以此类推 F 表示的就是十进制的 15 了（A-F 不区分大小写）。在编程编程语言中使用 0x 或 0X 作为前缀表示十六进制。比如 0x1A 就是十进制的 26 了。

为了更方便于理解进制的管理我们来使用一个 table 表格看下 0-20 在十进制、二进制、八进制以及十六进制中分别如何表示：
